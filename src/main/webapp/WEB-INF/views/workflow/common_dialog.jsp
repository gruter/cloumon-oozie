			...
			<div>
				<div id="job_status_tab_container">
					...
					<div id="job_status_tab_1">
						<div style="height:545px; width:450px">
...
$("#job_status_tab_container").tabs();
$('#job_status_tab_container').bind('tabsshow', function(event, ui) {
	if(ui.index == 2) {
		workflowUtil.showJobLog();
	}
});
			...
			$.ajax({
				...
				success: function(result) {
					if(!result.success) {
						...
						workflowUtil.showJobDetailView(result.data);
					}
				}
			});			
		}
...
$("#job_status_job_property_table").jqGrid({
	datatype: "local",
	height: 520,
	width: 440,
	rowNum: 100000,
   	colNames:['key', 'value'],
   	colModel:[
   		{ name:'key',index:'key', width:200, sortable:true, editable: false },
   		{ name:'value',index:'value', width:240, sortable:true, editable: false }
   	]
});
...
$("#actionListTable").jqGrid({
	...
	width: 500,
	rowNum: 100000,
   	colNames:['Action Id', 'Name', 'Type', 'Status', 'Transition', 'Start Time', 'End Time', 'errorCode', 
   	      	'errorMessage', 'externalId', 'externalStatus', 'consoleUrl', 'trackerUri', 'conf'],
   	colModel:[
   		...
   		{name:'conf',index:'conf', width:70, sortable:false, hidden: true, 
   			formatter: function(val) { if(val == null) return "";	return val.replace(/<script>/g,"[script]").replace(/<\/script>/g, "[/script]"); }}
   	],
   	onSelectRow: function(id) {
		...
		var actionXml = $("#actionListTable").jqGrid('getCell', id, 'conf');
		if(actionXml != null) {
			actionXml = actionXml.replace(/\[script\]/g,"<script>").replace(/\[\/script\]/g, "<\/script>");
		}
		$("#job_status_action_xml").val(actionXml);
